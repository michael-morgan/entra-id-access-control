...
document.addEventListener("DOMContentLoaded", function () {
  ...
  clearButton.addEventListener("click", function () {
    ...
    const evaluationTraceSection = document.getElementById(
  ...
  function displayResults(result) {
    ...
    displayGroups(result.groupsWithNames || result.groups);
  ...
  function displayTokenClaims(claims) {
    ...
    for (const [key, value] of Object.entries(claims)) {
      container.innerHTML += `
                ...
                <dd class="col-sm-8 small"><code>${escapeHtml(
                  value
                )}</code></dd>
            `;
    }
  ...
  function displayGroups(groups) {
    ...
    if (groups && groups.length > 0) {
      // Check if groups have displayName property (GroupReference objects)
      container.innerHTML = groups
        .map((g) => {
          const displayText = g.displayName || g;
          const title = g.id ? `title="${escapeHtml(g.id)}"` : "";
          return `<span class="badge bg-primary me-1" ${title}>${escapeHtml(
            displayText
          )}</span>`;
        })
        .join("");
  ...
  function displayRoles(roles) {
    ...
    if (roles && roles.length > 0) {
      container.innerHTML = roles
        .map(
          (r) => `<span class="badge bg-success me-1">${escapeHtml(r)}</span>`
        )
        .join("");
    } else {
  ...
  function displayMergedAttributes(attributes) {
    ...
    if (!attributes || Object.keys(attributes).length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="3" class="text-muted text-center">No attributes</td></tr>';
      return;
    }
    ...
    for (const [name, attr] of Object.entries(attributes)) {
      ...
      // Display friendly name if available, fallback to entityId
      const entityDisplay = attr.entityName || attr.entityId || "";
      const entityTitle =
        attr.entityName && attr.entityId
          ? `title="${escapeHtml(attr.entityId)}"`
          : "";
      ...
      tbody.innerHTML += `
        ...
        attr.source
      )}</span> <span ${entityTitle}>${escapeHtml(entityDisplay)}</span></td>
                </tr>
            `;
    }
  }
  ...
  function displayUserAttributes(attributes) {
    ...
    if (!attributes || attributes.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="2" class="text-muted text-center">No user attributes</td></tr>';
      return;
    }
  ...
  function displayRoleAttributes(attributes) {
    ...
    if (!attributes || attributes.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="3" class="text-muted text-center">No role attributes</td></tr>';
      return;
    }
  ...
  function displayGroupAttributes(attributes) {
    ...
    if (!attributes || attributes.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="3" class="text-muted text-center">No group attributes</td></tr>';
      return;
    }
  ...
  function displayPolicies(policies) {
    ...
    if (!policies || policies.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="3" class="text-muted text-center">No applicable policies</td></tr>';
      return;
    }
    ...
    policies.forEach((policy) => {
      ...
      tbody.innerHTML += `
                <tr>
                    <td><span class="badge bg-${typeClass}">${escapeHtml(
        policy.policyType
      )}</span></td>
  ...
  function displayAbacRules(rules) {
    ...
    if (!rules || rules.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="2" class="text-muted text-center">No ABAC rules</td></tr>';
      return;
    }
    ...
    rules.forEach((rule) => {
      tbody.innerHTML += `
                <tr>
                    <td><span class="badge bg-dark">${escapeHtml(
                      rule.ruleType
                    )}</span></td>
  ...
  function displayRuleGroups(groups) {
    ...
    if (!groups || groups.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="3" class="text-muted text-center">No rule groups</td></tr>';
      return;
    }
  ...
  function displayDynamicScenarios(scenarios, token, workstreamId) {
    ...
    if (!scenarios || scenarios.length === 0) {
      scenariosContainer.innerHTML =
        '<div class="col-md-12"><p class="text-muted">No scenarios available for this user in this workstream.</p></div>';
      return;
    }
  ...
  function initializeCustomTestBuilder() {
    ...
    runCustomTestButton.addEventListener("click", async function () {
      ...
      if (!resource || !action) {
        showError(
          "Please provide both resource and action for the custom test."
        );
        return;
      }
  ...
  function displayCustomTestResult(result) {
    ...
    const customTestSection = document
      .getElementById("customEntityJson")
      .closest(".card-body");
    let resultAlert = customTestSection.querySelector(
      ".custom-test-result-alert"
    );
    ...
    resultAlert.className = "alert custom-test-result-alert mt-3";
    resultAlert.classList.add(
      result.isAuthorized ? "alert-success" : "alert-danger"
    );
    ...
    document
      .getElementById("evaluationTraceSection")
                ...
                <div class="d-flex align-items-start">
                    ...
                    <div class="flex-grow-1">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span class="badge bg-${resultClass}">${escapeHtml(
        step.result
      )}</span>
                        </div>
                        <p class="mb-1 text-muted">${escapeHtml(
                          step.description
                        )}</p>
                        ${
                          step.details
                            ? `<div class="mt-2"><code class="small">${escapeHtml(
                                step.details
                              )}</code></div>`
                            : ""
                        }
